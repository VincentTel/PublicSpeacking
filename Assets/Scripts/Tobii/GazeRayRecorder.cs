...
public class GazeRayRecorder : MonoBehaviour
{
    ...
    private GazeRayRenderer gazeRayRenderer = null;

    private string filePath;

    public static GazeRayRecorder Instance
    {
        get
        {
            return instance;
    ...
    private string GetEyeTrackingInfo()
    {
        ...
        Vector3 o = gazeRayRenderer.GetRayOrigin(); // origin
        Vector3 d = gazeRayRenderer.GetRayDirection(); // direction
    ...
    void Update()
    {
        if (isRecording)
        {
            ...
            if (sGazeFrame.Length > 0)
            {
                ...
                using (StreamWriter writer = File.AppendText(filePath + fileName))
                {
                    ...
                    writer.WriteLine(sRelTime + "|" + sGazeFrame);
        ...
        if (isPlaying)
        {
            ...
            {
                // then play the line
                if (gazeRayRenderer && sPlayLine.Length > 0)
                {
                    gazeRayRenderer.SetGazeData(sPlayLine);
                }
